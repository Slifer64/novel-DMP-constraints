...
t_g = inf; % so that the target doesn't change
yg0 = yg;
...
dmp_mpc  = 1; % MPC GMP
traj_mpc = 1; % MPC with reference from DMP
...
function [Time, P_data, dP_data, ddP_data] = mpcTrajOpt(gmp0, tau, mpc_dt, y0, yg, pos_lim, vel_lim, accel_lim, opt_pos, opt_vel, qp_solver_type)
    ...
    final_state_err_tol = [1e-3*ones(n_dof,1); 1e-2*ones(n_dof,1)];
...
%     Z_lb((N-1)*n+1 : n*N) = [x_final(1:n_dof); x_final(n_dof+1:2*n_dof)] - final_state_err_tol;
%     Z_ub((N-1)*n+1 : n*N) = [x_final(1:n_dof); x_final(n_dof+1:2*n_dof)] + final_state_err_tol;
    ...
    while (true)
        ...
        if (s <= 1), text_prog.update(100*s); end
        ...
        for i=1:N
            ...
            if (si >= 1)
                ...
                if (~fc_flag)
                    Z_lb((i-1)*n+1:i*n) = x_final - final_state_err_tol;
                    Z_ub((i-1)*n+1:i*n) = x_final + final_state_err_tol;
                    fc_flag = 1;
                end
                
                Qi_ = 1e4*eye(size(Qi)); % add high cost for terminal state
...
%         qx((N-1)*n+1 : N*n) = -QN*[yd_i; dyd_i];
        ...
        H = sparse( blkdiag( Qi_cell{:}, kron(speye(N), Ri) ) );
        ...
        if (qp_solver_type == 1)
            ...
            if (~osqp_initialized)
                % Setup workspace
                prob.setup(H, q, A_osqp, lb, ub, 'warm_start',true, 'verbose',false, 'eps_abs',1e-4, 'eps_rel',1e-6, 'max_iter',30000);
