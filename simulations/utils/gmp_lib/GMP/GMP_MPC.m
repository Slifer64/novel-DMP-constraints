...
%                 ub_vp = [this.vel_ub; this.accel_ub];
        ...
        function plot_obstacle_constr(this, si_data)
            ...
            P_next = zeros(this.n_dof, length(s_next));
            for j=1:length(s_next), P_next(:,j) = this.getYd(s_next(j)); end
            h = plot(P_next(1,:), P_next(2,:), 'LineWidth',2, 'Color',[1 0.6 1]);
            this.plt_handles = [this.plt_handles h];
            ...
            for i=1:length(this.temp_s)
                ...
                h1 = quiver(p2(1), p2(2), n_e(1), n_e(2), 0.08, 'Color','green', 'LineWidth',3, 'Parent',this.ax);
                h2 = plot([p3(1) p4(1)], [p3(2) p4(2)], 'LineWidth',2.2, 'LineStyle','-.', 'Color',[0 0.8 0], 'Parent',this.ax);
                h3 = plot([c(1) p2(1)], [c(2) p2(2)], 'LineWidth',2, 'LineStyle',':', 'Color','cyan', 'Parent',this.ax);
                h4 = plot(p1(1), p1(2), 'LineWidth',2, 'Marker','x', 'LineStyle','None', 'Markersize',14, 'Color','red', 'Parent',this.ax);
                % h5 = plot(p2(1), p2(2), 'LineWidth',2, 'Marker','x', 'LineStyle','None', 'Markersize',14, 'Color','magenta', 'Parent',this.ax);
                % axis(this.ax, 'equal');
            ...
            for j=1:size(this.yd_points, 2)
                h = plot(this.yd_points(1,j), this.yd_points(2,j), 'LineWidth',2, 'Marker','x', 'LineStyle','None', 'Markersize',14, 'Color','cyan', 'Parent',this.ax);
                this.plt_handles = [this.plt_handles h];
            end
            
            for j=1:size(this.y_points, 2)
                h = plot(this.y_points(1,j), this.y_points(2,j), 'LineWidth',2, 'Marker','o', 'LineStyle','None', 'Markersize',14, 'Color','magenta', 'Parent',this.ax);
                this.plt_handles = [this.plt_handles h];
            end
...
%             end
            ...
            this.plot_obstacle_constr(si_data);
    ...
    properties (Access = public)
        ...
        P_data
        ax
        p_h = plot(nan, nan, 'LineWidth',2, 'Color','magenta') % handle for the path executed so far
        temp_s
        yd_points = []
        y_points = []
        plt_handles = []
