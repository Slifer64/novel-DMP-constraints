...
%
% based on "A novel approach to dynamic movement imitation based on
% quadratic programming" , doi: 10.1109/ICRA.2015.7139285
...
classdef QPMP < matlab.mixin.Copyable
    ...
    methods (Access = public)
        ...
        function this = QPMP(N_kernels, a_z, b_z, kernel_std_scaling)
            ...
            this.c = linspace(0,1, this.N_kernels)';
...
%             this.w = zeros(this.N_kernels,1);
        ...
        %% Sets the demo data.
        function setDemo(this, Timed, demo_accel, taud, yd0, gd)
        ...
        function [success, exit_status] = train(this, tau, y0, ydot_0, g, pos_constr, vel_constr, accel_constr)
...
%             t_start = tic;
            ...
            [w_opt, ~, ex_flag, opt_output] = quadprog(sparse(H),f, sparse(A),b, Aeq,beq, [],[], this.w, opt);
            exit_status = opt_output.message;
...
%             disp([ 'A-sp: ' num2str(sum(A(:)== 0)) ' / ' num2str(numel(A)) ] );
            
%             n_constr = length(accel_constr);
%             
%             t_lb = [];
%             value_lb = [];
%             t_ub = [];
%             value_ub = [];
%             for j=1:n_constr
%                 if (accel_constr(j).type == '<')
%                     t_lb = [t_lb accel_constr(j).t];
%                     value_lb = [value_lb accel_constr(j).value];
%                 elseif (accel_constr(j).type == '>')
%                     t_ub = [t_ub accel_constr(j).t];
%                     value_ub = [value_ub accel_constr(j).value];
%                 end
%             end
%             
%             
%             t_data = t_lb;
%             Accel = zeros(1,length(t_data));
%             for j=1:length(t_data)
%                 x = t_data(j) / tau;
%                 Accel(j) = dot(this.regressVec(x), this.w);
%             end
%             
%             figure; hold on;
%             plot(t_lb, value_lb, 'LineStyle','--', 'Color','red','LineWidth',1.2);
%             plot(t_ub, value_ub, 'LineStyle','--', 'Color','red','LineWidth',1.2);
%             plot(t_data, Accel, 'LineStyle','-', 'Color','blue','LineWidth',1.6);
%             
%             stop
