function [Time, P_data, dP_data, ddP_data] = gmpWithRepulsiveForces(gmp0, tau, y0, yg0, yg, t_g, pos_lim, vel_lim, accel_lim)
    
    global K D kp kv log_data pos_on vel_on dt
...
%     [Time, P_data, dP_data, ddP_data] = simEuler(gmp, tau, y0, y_min, y_max, ydot_min, ydot_max);
    [Time, P_data, dP_data, ddP_data] = simEulerWithOdeStep(gmp, tau, y0, yg, t_g, y_min, y_max, ydot_min, ydot_max);
...
function [Time, P_data, dP_data, ddP_data] = simEuler(gmp, tau, y0, y_min, y_max, ydot_min, ydot_max)
    ...
    global K D kp kv pos_on vel_on dt
...
%         fp = kp ./ ( (1 - y./y_max) .* (y./y_min - 1) );
        ...
        z_dot = -K*(y - yd) - D*(z - yd_dot) + yd_ddot + vel_on*fv + pos_on*fp;
        ydot = z; % + 0*fp;
...
function [Time, P_data, dP_data, ddP_data] = simEulerWithOdeStep(gmp, tau, y0, yg, t_g, y_min, y_max, ydot_min, ydot_max)
...
function s_dot = ode_fun(t, s, gmp, tau, y0, y_min, y_max, ydot_min, ydot_max)
    ...
    global K D kp kv pos_on vel_on
