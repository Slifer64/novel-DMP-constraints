function [Time, P_data, dP_data, ddP_data] = mpcTrajOpt(gmp0, tau, y0, yg, pos_lim, vel_lim, accel_lim, opt_pos, opt_vel, qp_solver_type)
    ...
    final_state_err_tol = [1e-2*ones(n_dof,1); 1e-1*ones(n_dof,1)];
    
    %% --------  Init MPC  --------
    N = 10;%10; %200;
    dt_ = 0.01; %dt;%dt;%0.015;%dt; %0.010;
    use_ud = 0;
...
%                 %if (i==N-1)
                ...
                %QN = 1e6*eye(size(Qi)); % add high cost for final state
                yd_i = yg;
                dyd_i = zeros(n_dof,1);
